  ...
  try {
    ...
    try {
      ...
      const incResult = await client.query(`
        INSERT INTO equipment_incidents (equipment_id, title, description, root_cause, solution, status, reported_by)
...
router.patch('/:id', requireRole('admin', 'moderator', 'storekeeper'), async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE equipment_incidents SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
